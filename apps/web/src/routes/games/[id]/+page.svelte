...
<section class="mb-8 grid grid-cols-2 gap-3 sm:grid-cols-4" aria-label="Final scores">
  {#each data.game.players as player (player.seat)}
    <article class={`border-l-[3px] border-y border-r border-border-subtle bg-surface-1/60 p-3 ${seatBorders[player.seat] ?? 'border-l-border'}`}>
      <div class="mb-3 flex min-w-0 items-center gap-2">
        ...
        {#if player.seat === data.game.kyoku[0]?.dealer}
          <span class="stamp-tag shrink-0 border-gold text-gold" lang="ja">親 · OYA</span>
        {/if}
      </div>
      ...
      <dl class="mt-3 grid grid-cols-2 gap-x-2 gap-y-1 text-[11px] leading-[15px]">
        <dt class="text-text-tertiary">Win</dt><dd class="text-right font-mono text-sou">{formatPercent(player.stats.rates.winRate)}</dd>
        <dt class="text-text-tertiary">Deal-in</dt><dd class="text-right font-mono text-man">{formatPercent(player.stats.rates.dealInRate)}</dd>
        <dt class="text-text-tertiary">Riichi</dt><dd class="text-right font-mono text-gold">{formatPercent(player.stats.rates.riichiRate)}</dd>
      </dl>
    </article>
  {/each}
</section>
